...
const screenForNames = (arr) => {
  let regex = /^(?:Mrs\.\s?|Mr\.\s?|Dr\.\s?|Ms\.\s?)\w+$/gm;
...
const toTitleCase = (arr) => {
  ...
  arr.forEach( str => {
    newArr.push(str.charAt(0).toUpperCase() + str.substr(1));
  })
...
const sortBy = (property, arr) => {
 ...
 let sorted = arr.sort((curr , prev ) => {
  return curr[property] < prev[property] ? -1 : 1
 })
return sorted;
};
