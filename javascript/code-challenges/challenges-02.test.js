...
const raisedToTheThird = (arr) => {
  const newArray = [];
  arr.forEach(toThePower => newArray.push(Math.pow(toThePower, 3)))
  return newArray;
};
...
const addOne = (arr) => {
  ...
  let plusOne = arr.map(number => (number + 1));
  return plusOne;
};
...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach(powerOfTwo => newArray.push(Math.pow(2, powerOfTwo)))
  return newArray;
};
...
const mapTwoToThe = (arr) => {
  let powerOf = arr.map(powerOfTwo => (Math.pow(2, powerOfTwo)))
  return powerOf;
};
