...
const typeNum = (arr) => {
 return arr.filter(banans => {return typeof(banans) === 'number' });
};
...
const containsAnd = (arr) => {
 return arr.filter (str => {return str.includes('and')});
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(content => !(forbiddenValues.includes(content)));
};
